...
mod mock_routing {
    ...
    async fn concurrent_revocation_of_single_app() -> Result<(), AuthError> {
        ...
        let app_0 = unwrap!(verify_app_is_revoked(&client, app_id_0, ac_entries_0).await);
...
// balance.
#[tokio::test]
async fn flushing_empty_app_revocation_queue_does_not_mutate_network() {
    // Create account.
...
#[tokio::test]
async fn revocation_with_unencrypted_container_entries() {
    let (auth, ..) = create_authenticator().await;
    let client = auth.client;
